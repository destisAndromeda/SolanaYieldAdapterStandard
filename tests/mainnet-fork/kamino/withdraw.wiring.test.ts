...
describe("kamino mainnet-fork withdraw wiring", () => {
  before(async () => {
    await setupKaminoFork();
  });

  it("should build dispatcher.withdraw with the expected Kamino remaining accounts", async () => {
    ...
    const dispatcherIx = await dispatcherProgram.methods
      .withdraw({ amount: withdrawAmount, extraData })
      .accounts(accounts({
