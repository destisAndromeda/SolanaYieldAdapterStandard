...
import {
  ...
  USER_USDC_ATA,
  jupiterAdapterPda,
  jupiterDepositRemainingAccounts,
  provider,
  setupJupiterFork,
} from "./shared";
...
describe("jupiter mainnet-fork deposit wiring", () => {
  before(async () => {
    await setupJupiterFork();
  });

  it("should build dispatcher.deposit with the expected Jupiter remaining accounts", async () => {
    const amount: anchor.BN = new anchor.BN(1_000_000);
    const extraData = Buffer.from([0]);
    const remainingAccounts = jupiterDepositRemainingAccounts();
